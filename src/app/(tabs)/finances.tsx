...
import { createSalaryPeriod, registerSalaryPayment } from "@/modules/salary/service";
import {
...
export default function FinancesScreen() {
    ...
    try {
      ...
      if (sheet === "transfer") {
        ...
        if (
          ...
          setError("La transferencia debe ser entre monedas activas diferentes.");
          setIsSubmitting(false);
          return;
        }
  ...
  const quickItems = [
    ...
    { icon: "repeat", label: "Transferir Moneda", tone: styles.quickOrange, onPress: () => openSheet("transfer") },
    { icon: "calendar-outline", label: "Registrar Nomina", tone: styles.quickBlue, onPress: () => openSheet("salary-period") },
    { icon: "wallet-outline", label: "Registrar Salario", tone: styles.quickBlue, onPress: () => openSheet("salary-payment") },
  ];
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <BottomSheet onClose={closeQuickSheet} visible={sheet === "quick"}>
        ...
        <View style={styles.quickGrid}>
          {quickItems.map((item) => (
            <Pressable key={item.label} onPress={item.onPress} style={({ pressed }) => [styles.quickTile, pressed && styles.pressed]}>
              <View style={[styles.quickIcon, item.tone]}>
                {item.icon === "wallet-outline" || item.icon === "calendar-outline" ? (
                  <Ionicons color="#4B69FF" name={item.icon as "wallet-outline" | "calendar-outline"} size={22} />
                ) : (
                  <Feather color={item.icon === "arrow-up-right" ? "#FF6B6D" : item.icon === "arrow-down-left" ? "#20D396" : "#F59E0B"} name={item.icon as "arrow-down-left" | "arrow-up-right" | "repeat"} size={22} />
                )}
              </View>
      ...
      <BottomSheet onClose={closeFormSheet} visible={activeFormSheet !== null}>
        ...
        <Text style={styles.label}>{sheet === "salary-period" ? "MONTO NOMINA" : "MONTO"}</Text>
        ...
        {sheet === "salary-payment" ? <Text style={styles.soft}>Se asignara automaticamente a los periodos pendientes visibles.</Text> : null}
        {sheet === "salary-period" ? <Text style={styles.soft}>Crea un periodo de nomina para que los cobros de salario se asignen automaticamente.</Text> : null}
